...
if(isset($_SESSION['username']) && isset($_SESSION['password'])){
	...
	$query=mysqli_query($con,"SELECT * FROM events WHERE user!='admin'");
	...
	if($row!=0){
		...
		$per_page=5;
		$pages_query=mysqli_query($con,"SELECT COUNT('id') FROM events WHERE user!='admin'");
		#$pages=ceil(mysql_result($pages_query,0)/$per_page);
		$pages=ceil($row/$per_page);
		$page=(isset($_GET['page'])) ? (int)$_GET['page'] : 1;
		$start=($page-1)*$per_page;		
		$query=mysqli_query($con,"SELECT * FROM events WHERE user!='admin' ORDER BY date DESC LIMIT $start,$per_page");
